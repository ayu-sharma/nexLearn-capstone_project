...
// Defining proper component props interfaces
...
const Courses: React.FC = () => {
  ...
  const calculateProgress = () => {
    ...
    return totalMaterials > 0
      ? (completedMaterials.length / totalMaterials) * 100
      : 0;
  };
    ...
    const fetchCourse = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:3000/api/courses/fetch/${courseId}`
        );
        ...
        const completedRes = await axios.get(
          `http://localhost:3000/api/courses/completed/${courseId}`,
          {
            headers: {
        ...
        if (courseDetails?.modules?.length > 0) {
          const allMaterials: Material[] = courseDetails.modules.flatMap(
            (module: Module) => module.materials
          );
          const firstIncompleteMaterial = allMaterials.find(
            (material: Material) =>
  ...
  const handleNextMaterial = () => {
    ...
    const currentModuleIndex = course.modules.findIndex((module) =>
      module.materials.some((material) => material._id === currentMaterial._id)
    );

    if (currentModuleIndex === -1) return;

    const currentModule = course.modules[currentModuleIndex];
    const currentMaterialIndex = currentModule.materials.findIndex(
      (material) => material._id === currentMaterial._id
    );
  ...
  const handlePrevMaterial = () => {
    ...
    if (currentMaterialIndex > 0) {
      ...
      if (prevModule.materials.length > 0) {
        const prevMaterial =
          prevModule.materials[prevModule.materials.length - 1];
        handleMaterialClick(prevMaterial);
      }
    }
  };
  ...
  const handleMarkAsCompleted = async () => {
    ...
    try {
      await axios.post(
        `http://localhost:3000/api/courses/toggle`,
        {
          // Fixed URL with full path
          courseId: courseId,
      ...
      >
        ...
        <div className="p-4">
          ...
          {course?.modules.map((module, index) => (
            <div
              key={index}
              className="mb-4 border dark:border-neutral-700 border-neutral-200 rounded-lg overflow-hidden"
            >
              <button
          ...
          <div className="my-6">
            ...
            <div className="mt-6 mb-8">
              ...
              {currentMaterial.type === "READING" &&
                currentMaterial.textContent && (
                  <div className="prose dark:prose-invert max-w-none prose-img:mx-auto prose-img:rounded-xl prose-img:max-w-full">
                    <ReadingContent content={currentMaterial.textContent} filename={currentMaterial.title}/>
                  </div>
                )}
              ...
              <Button
                onClick={() => {
                  ...
                  if (
                    ...
                    course.modules[0].materials.length > 0
                  ) {
